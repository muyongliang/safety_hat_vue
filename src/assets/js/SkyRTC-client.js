const SkyRTC = function () {
    ...
    const iceServer = {
        "iceServers": [
            ...
            {
                "url": "stun:47.104.189.23:3478"
            },
            {
                "url": "turn:47.104.189.23:3478",
                "username": "pyf",
                "credential": "123456"
            },
    ...
    function skyrtc() {
        ...
        this.on('_new_peer', function (data) {
            ...
            if (that.localMediaStream !== null) {
                ...
                that.emit('new_peer', data.stream, data.socketId);
            } else {
    ...
    skyrtc.prototype.connect = function (server, room) {
        ...
        socket.onclose = function (data) {
            ('websocket 断开: ' + data.code + ' ' + data.reason + ' ' + data.wasClean)
            // if (data.code == 1006) {
            ...
            that.localMediaStream.getTracks()[0].stop();
            var pcs = that.peerConnections;
            if (that.connections.length) {
                for (let i = that.connections.length - 1; i >= 0; i--) {
                    that.closePeerConnection(pcs[that.connections[i]]);
                }
            }
            //  else {
            //      that.closePeerConnection(pcs[that.connections]);
            //  }
            var tracks = that.localMediaStream.getTracks();
    ...
    // 挂断
    skyrtc.prototype.closePC = function () {
        this.socket.close();
        this.socket = null;
    };
